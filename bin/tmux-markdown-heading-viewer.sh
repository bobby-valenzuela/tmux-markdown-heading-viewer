...
if [[ -s $CURRENT_DIR/markdown_dir.txt && "${1}" != 'reset' ]]; then
    ...
    MARKDOWN_DIR=$(
        if command -v fd >/dev/null && command -v tree >/dev/null; then
            fd --type d --hidden --follow --exclude .git . /home/ 2>/dev/null | fzf --preview 'tree -C {}' --header 'Choose a folder' --preview-window=down:30%
        else
...
if command -v fd >/dev/null; then
    FILE="$(fd -H --type file -e md -e markdown . ${MARKDOWN_DIR} | fzf --header "${MSG}" --preview-window down:60% --preview 'glow -p --width=80 {} ' )"
else
