...
if [[ -s $CURRENT_DIR/markdown_dir.txt && "${1}" != 'reset' ]]; then
    ...
    MARKDOWN_DIR=$(
        if command -v fd >/dev/null && command -v tree >/dev/null; then
            fd --type d --hidden --follow --exclude .git . /home/ 2>/dev/null | fzf $(print ${FZF_DEFAULT_OPTS}) --layout=reverse --preview 'tree -C {}' --header 'Choose a folder' --preview-window=down:40%
        else
            find /home/ -mindepth 1 -type d -not -path '*/\.*' 2>/dev/null | fzf ${FZF_DEFAULT_OPTS})  --layout=reverse --preview 'ls -la --color=always {}' --preview-window=down:40% --header 'Choose a folder'
        fi
    )
...
print_md_section(){
    ...
    HEADING="$(grep -E '^\s{0,3}#+' $1 | fzf ${FZF_DEFAULT_OPTS}) --layout=reverse -m --preview "grep -A 100 {} $1 " --preview-window down:50% --header "${MSG}")"
...
if command -v fd >/dev/null; then
    FILE="$(fd -H --type file -e md -e markdown . ${MARKDOWN_DIR} | fzf ${FZF_DEFAULT_OPTS})   --layout=reverse --header "${MSG}" --preview-window down:60% --preview 'glow -p --width=80 {} ' )"
else
